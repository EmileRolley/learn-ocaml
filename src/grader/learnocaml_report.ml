           ...
           case
             (obj2 (req "text" string)
                (dft "display"
                   (string_enum
                      [
                        ("normal", `Normal); ("code", `Code); ("output", `Output); ("image", `Image)
                      ])
                   `Normal))
             (function
    ...
    union
      [
        case int
          ...
          (fun n ->
            if n > 0 then Success n else if n < 0 then Penalty (-n) else Failure);
        case
          (string_enum
             [
               ("failure", Failure);
...
let folder, unfolder =
  ...
  ( E
      ( "span",
        [ ("onclick", js); ("class", "folder-icon clickable") ],
        [ T "v" ] ),
    E
      ( "span",
        [ ("onclick", js); ("class", "folder-icon clickable") ],
...
let format items =
  ...
  and format_item = function
    ...
    | Section (title, contents) -> format_section title (format_report contents)
  ...
  and format_text text =
    let format = function
      ...
      | Image _ ->
          ...
          (* let r = Vgr.create (Vgr_svg.target ()) (`Buffer buff) in *)
          (* ignore (Vgr.render r (`Image (size, view, i))); *)
          (* ignore (Vgr.render r `End); *)
          (* S ("img", [ ("src", Buffer.contents buff) ]) *)

          (* FIXME: cannot reach this case.. *)
          T "Will be an vg image"
        ...
        E
          ( "div",
            ...
            [
              E
                ( "span",
                  [ ("class", "title clickable"); ("onclick", js) ],
                  score );
            ] );
...
let print ppf items =
  ...
  and print_text ppf = function
    ...
    | Text wa :: Text wb :: rest -> print_text ppf (Text (wa ^ " " ^ wb) :: rest)
    | Text w :: rest ->
        Format.fprintf ppf "@[<hov>%a@]%a" Format.pp_print_text w print_text
          rest
    | Break :: rest -> Format.fprintf ppf "@\n%a" print_text rest
    | Code s :: rest -> Format.fprintf ppf "@,%a%a" print_code s print_text rest
    | Output s :: rest ->
        Format.fprintf ppf "@,%a%a" print_code s print_text rest
    | Image _ :: rest -> Format.fprintf ppf "Vg image%a" print_text rest
    | [] -> ()
